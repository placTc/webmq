...
pub fn get_file_length(file: &Path) -> Result<usize, WebMQError> {
    ...
    let file_meta = match file_meta {
        Ok(meta) => meta,
        Err(e) => {
            return Err(WebMQError::File(format!("Couldn't get length of file {}: {}", file.to_string_lossy(), e)));
        }
    };
...
pub fn get_file_buffer(file: &Path) -> Result<Vec<u8>, WebMQError> {
    let cert_file_length = match get_file_length(file) {
        Ok(len) => len,
        Err(e)  => {
            return Err(WebMQError::Config(e.to_string()))
        }
    };
    ...
    .and_then(|mut file| {
        ...
        Err(e) => Err(WebMQError::File(format!("Couldn't open file {}: {}", file.to_string_lossy(), e))),
        Ok(vec) => Ok(vec)
    }
}